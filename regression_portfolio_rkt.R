...
################################# 
#### The Regression Part
load("F:/我的论文/第五篇/主代码/realized kurtosis puzzle/RData/da_p_50.RData")
#load("F:/我的论文/第五篇/主代码/realized kurtosis puzzle/RData/da_p_100.RData")
#load("F:/我的论文/第五篇/主代码/realized kurtosis puzzle/RData/da_p_200.RData")
...
####  
lm_1 <- lmList(ret_e ~ rkt | ym , data=da_p)
lm_fit_nw(lm_1,1) 
lm_2 <- lmList(ret_e ~ rkt+be+size+BM | ym , data=da_p)
lm_fit_nw(lm_2,4)
lm_3 <- lmList(ret_e ~ rkt+be+size+BM+mom | ym , data=da_p)
lm_fit_nw(lm_3,5)
lm_4 <- lmList(ret_e ~ rkt+be+size+BM+mom+ret_e_tm+illiq | ym , data=da_p)
lm_fit_nw(lm_4,7)

lm_5 <- lmList(ret_e ~ max_ret | ym , data=da_p)
lm_fit_nw(lm_5,1)
lm_6 <- lmList(ret_e ~ rvol | ym , data=da_p)
lm_fit_nw(lm_6,1)
lm_7 <- lmList(ret_e ~ rsk | ym , data=da_p)
lm_fit_nw(lm_7,1)

lm_8 <- lmList(ret_e ~ rkt+max_ret | ym , data=da_p)
lm_fit_nw(lm_8,2)
lm_9 <- lmList(ret_e ~ rkt+rvol | ym , data=da_p)
lm_fit_nw(lm_9,2) 
lm_10 <- lmList(ret_e ~ rkt+rsk | ym , data=da_p)
lm_fit_nw(lm_10,2)
lm_11 <- lmList(ret_e ~ rkt+max_ret+rvol+rsk | ym , data=da_p)
lm_fit_nw(lm_11,4)
