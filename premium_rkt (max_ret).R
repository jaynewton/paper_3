...
da_premium <- merge(da_premium_rkt,da_premium_max,by="ym")
model <- lm(premium_rkt ~ premium_max,data=da_premium)
summary(model)
coeftest(model,vcov = NeweyWest(model))[2,3]

da_premium_max <- da_premium[,.(ym,premium_max)]
save(da_premium_max,file="C:/Users/Ding/Desktop/da_premium_max.RData")
...
load("F:/我的论文/第五篇/RData/da_all_m.RData")
load("F:/我的论文/第五篇/RData/da_realized_m.RData")
load("F:/我的论文/第五篇/RData/da_premium_max.RData")
load("F:/我的论文/第五篇/RData/FF3F_A_nm.RData")

da_premium <- copy(da_premium_max)
