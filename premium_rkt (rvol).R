...
lm_rvol <- lmList(ret_e ~ rvol | ym , data=da_all_m)
da_premium_rvol <- data.table(ym=ymd(names(summary(lm_rvol)$coefficients[,,2][,1])),
                             premium_rvol=as.numeric(summary(lm_rvol)$coefficients[,,2][,1]))
da_premium_rvol[,premium_rvol:=-premium_rvol]

da_premium <- merge(da_premium_rkt,da_premium_rvol,by="ym")
model <- lm(premium_rkt ~ premium_rvol,data=da_premium)
summary(model)
coeftest(model,vcov = NeweyWest(model))[2,3]

da_premium_rvol <- da_premium[,.(ym,premium_rvol)]
save(da_premium_rvol,file="C:/Users/Ding/Desktop/da_premium_rvol.RData")
...
load("F:/我的论文/第五篇/RData/da_all_m.RData")
load("F:/我的论文/第五篇/RData/da_realized_m.RData")
load("F:/我的论文/第五篇/RData/da_premium_rvol.RData")
load("F:/我的论文/第五篇/RData/FF3F_A_nm.RData")

da_premium <- copy(da_premium_rvol)

da_premium[,rvol_high:=ifelse(premium_rvol>=median(premium_rvol),1,0)]
da_premium <- da_premium[,.(ym,rvol_high)]
...
#median(da_all_m[,premium_rvol]) 

da_all_m_1 <- da_all_m[rvol_high==1,]
da_all_m_2 <- da_all_m[rvol_high==0,]
