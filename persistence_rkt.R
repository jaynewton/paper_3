...
da_nm <- da_m[,.(SecCode=SecCode,ym=ym-months(1),rkt_nm=rkt)]
#da_nm <- da_m[,.(SecCode=SecCode,ym=ym-months(2),rkt_nm=rkt)]
#da_nm <- da_m[,.(SecCode=SecCode,ym=ym-months(3),rkt_nm=rkt)]
#da_nm <- da_m[,.(SecCode=SecCode,ym=ym-months(6),rkt_nm=rkt)]
#da_nm <- da_m[,.(SecCode=SecCode,ym=ym-months(12),rkt_nm=rkt)]
...
#################################
# the proportion of stocks with the top 20%(10%,5%) rkt value this month
# still staying in the top 20% next month

## top 20%
da_transition_1 <- da_m
k <- 5
FUN_GROUP(da_transition_1,"rkt",k,0)
FUN_GROUP(da_transition_1,"rkt_nm",k,1)
# the next month
...
k <-10
FUN_GROUP(da_transition_2,"rkt",k,0)
FUN_GROUP(da_transition_2,"rkt_nm",k,1)
# the next month
transition <- matrix(NA,nrow=k,ncol=k)
for (i in 1:k) {
  for (j in 1:k) {
...
k <-20
FUN_GROUP(da_transition_3,"rkt",k,0)
FUN_GROUP(da_transition_3,"rkt_nm",k,1)
# the next month
transition <- matrix(NA,nrow=k,ncol=k)
...
lm_rkt_1 <- lmList(rkt_nm ~ rkt | ym , data=da_m)
lm_fit_nw(lm_rkt_1,1)
lm_rkt_1 <- lmList(rkt_nm ~ rkt+be+log(size)+log(BM) | ym , data=da_m_3F)
lm_fit_nw(lm_rkt_1,4)
