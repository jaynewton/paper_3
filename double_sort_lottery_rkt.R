#################################
#### Double Sort (Controlling Lottery Sales/GDP ratio)

load("F:/我的论文/第五篇/RData/da_all_m.RData")
load("F:/我的论文/第五篇/RData/da_realized_m.RData")
load("F:/我的论文/第五篇/RData/da_lottery_m.RData")
load("F:/我的论文/第五篇/RData/da_gdp_m.RData")
load("F:/我的论文/第五篇/RData/da_address.RData")

da_lottery_m <- merge(da_lottery_m,da_gdp_m,by=c("province","ym"))
da_lottery_m[,lottery:=lottery/gdp]
da_lottery_m[,gdp:=NULL]
da_lottery_m <- da_lottery_m[order(ym,province),]
da_lottery_m[,.(lottery=mean(lottery)),by=province][order(lottery),]

da_m <- merge(da_all_m,da_realized_m,by=c("ym","SecCode")) 
da_m <- merge(da_m,da_address,by="SecCode") 

#fsetequal(da_m[,.(sort(unique(province)))],da_lottery_m[,.(sort(unique(province)))])
#setequal(da_m[,sort(unique(province))],da_lottery_m[,sort(unique(province))])
#da_m[,sort(unique(province))]==da_lottery_m[,sort(unique(province))]

da_m <- merge(da_m,da_lottery_m,by=c("ym","province")) 
